def handles(list)
  puts "Combien y a t-il de handles ?"
  puts "Il y a #{list.size} handles"
end

def shortest_handle(list)
  puts "Quelle est le plus court handle de cette liste ?"
  puts "Le plus court handle est #{ list.min {|a,b| a.size <=> b.size}}"
end
...
def handle_alphabetic_order(list)
  ...
  p list.sort { |a, b| a.downcase <=> b.downcase}
end

def handle_size_order(list)
  puts "Voici la liste trié par rapport à leur taille (croissant)"
  p list.sort { |a, b| a.length <=> b.length}
end

def handle_epenser(list)
  puts "Quelle est la postion de @epenser dans la liste ?"
  puts "La position de @epenser dans la liste est #{list.index("@epenser")}"
end
...
def handle_div(list)
  ...
  while size <= list.max {|a,b| a.size <=> b.size }.size
    if list.select {|elem| elem.size == size}.size == 1
      ...
      puts "Il y a #{list.select {|elem| elem.size == size}.size} handles qui font #{size} caractères"
    end
    size += 1
  end
end
...
def perform
  ...
  handles(list)
  shortest_handle(list)
  handle5(list)
  handle_maj(list)
  handle_alphabetic_order(list)
  handle_size_order(list)
  handle_epenser(list)
  handle_div(list)
end
